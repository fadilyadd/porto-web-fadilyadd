  ...
  return (
    ...
    >
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={`absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl shadow-xl will-change-transform ${cardClassName ?? ""}`}
        ...
        {showNavigation && (
          <>
            <button type="button" aria-label="Previous slide" onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-black/70 p-2 text-white backdrop-blur transition hover:bg-black">
              <ChevronLeft className="size-5" />
            </button>
            <button type="button" aria-label="Next slide" onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-black/70 p-2 text-white backdrop-blur transition hover:bg-black">
      ...
      {showCaption && active?.title && (
        <div key={selected} className="mt-2 flex flex-col items-center px-6">
          <p className="text-[15px] font-semibold tracking-tight text-white">{active.title}</p>
          {active.subtitle && <p className="mt-1 text-[13px] text-zinc-400">{active.subtitle}</p>}
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          {slides.map((_, index) => (
            <button key={index} type="button" aria-label={`Go to slide ${index + 1}`} aria-current={index === selected}
              onClick={() => goTo(index)}
              className={`size-2 rounded-full transition-opacity ${index === selected ? "opacity-100 bg-white" : "opacity-30 bg-zinc-500"}`}
            />
          ))}
        </div>
      )}
